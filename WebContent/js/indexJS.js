...
$(document)
		.ready(
				function() {
>>>>>>> e42172ae78fcb9862c3900ea4cae5f586d3b361c

					var animationEnd = 'webkitAnimationEnd mozAnimationEnd MSAnimationEnd oanimationend animationend';

					if ($(window).width() < 580) {
						$("#attributes_panel").css(
								"height",
								$("#content").outerHeight(true)
										- $("#result").outerHeight(true));
						$("#cat2").children("span").attr("class", "after");
						$("#cat4").children("span").attr("class", "after");
					} else {
						$("#cat2").children("span").attr("class", "before");
					...
					$("#signupForm").submit(
							function() {
								...
								// PROCESS
								var encryptedPseudo = CryptoJS.AES.encrypt(
										pseudo, key);
								var encryptedMail = CryptoJS.AES.encrypt(mail,
										key);
								var encryptedMDP = CryptoJS.AES.encrypt(mdp,
										key);
								var encryptedVerifMDP = CryptoJS.AES.encrypt(
										mdp, key);
								var encryptedDate = CryptoJS.AES.encrypt(date,
										key);
								...
								$("#suPassword").val(encryptedMDP);
								$("#suDate").val(encryptedDate);
								$("#suVPassword").val(encryptedPasswordVerif); // Ou
																				// ""
																				// si
																				// on
																				// check
					...
					$("#attributes_form").change(
							function() {
								doPOST('UpdateResultsServlet', updateResults,
										$(this).serialize());
							});
					...
					$("#moreInfos")
							.click(
									function() {
										...
										$("#resultSection")
												.removeClass("summedResult")
												.addClass("detailedResult")
												.one(
														animationEnd,
														function() {
					...
					$("#showfilters").click(
							function() {
								...
								$("#attributes_panel, #attrPanelBackground")
...
$(window).on(
		...
		function() {
			...
			if ($(window).width() < 580) {
				...
				if ($("#resultSection").outerHeight(true) < $("#content")
						.outerHeight(true)) {
					$("#attributes_panel").css(
							"height",
							$("#content").outerHeight(true)
				...
				if ($("#resultSection").height() != $("#content").outerHeight(
						true)) {
				...
				$("#attributes_panel, #attrPanelBackground")
						.removeAttr('style').removeClass(
								'animated bounceOutLeft');
				$("#attributes_form").children("h4").children("span")
						.removeAttr('style').addClass("before");
...
function updateResults(data) {
	...
	$.each(data, function(index, city) {
		...
		$("#" + index + " > .cityWhereInfos").text(
				city.region
						+ " - "
						+ city.departement
						+ ($("#attributes_form > [name='distance']").prop(
								"checked") ? " - "
								+ city.details[0].taux_chomage : ""));
